...
function compareAll(reqStrs, scrapedStrs, compareTwoFn) {
	...
	let stringMap = {};
	let score1 = compareTwoFn(reqStrs[0], scrapedStrs[0]);
	let score2 = compareTwoFn(reqStrs[1], scrapedStrs[1]);
	stringMap[scrapedStrs[0]] = reqStrs[0];
	stringMap[scrapedStrs[1]] = reqStrs[1];
	let score3 = compareTwoFn(reqStrs[0], scrapedStrs[1]);
	let score4 = compareTwoFn(reqStrs[1], scrapedStrs[0]);
	if (score3 + score4 > score1 + score2) {
		stringMap[scrapedStrs[0]] = reqStrs[1];
		stringMap[scrapedStrs[1]] = reqStrs[0];
	} 
    if ((score1 === score3 && score2 === score4) || (score1 === score4 && score2 === score3)) { //dice's was inconclusive, fall back to levenshtein
		return compareAll(reqStrs, scrapedStrs, compareTwoLevenshtein);
	}
	return stringMap;
}
